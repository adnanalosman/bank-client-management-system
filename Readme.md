...
> ⚠️ **Note:** This path is Windows-specific and hardcoded. If you run the project on a different drive or OS, update the `Path` constant before building, or consider refactoring it to a relative path (see [Possible Improvements](#-possible-improvements)).
...
1. Open Visual Studio and create a new **Empty C++ Console Project**.
2. Add the source file (e.g., `main.cpp`) to the project.
3. Build the solution (`Ctrl+Shift+B`) and run (`Ctrl+F5`).
...
This project is open source. Consider adding a license (e.g., [MIT License](https://choosealicense.com/licenses/mit/)) to clarify how others may use your code — add a `LICENSE` file to the repository root.
